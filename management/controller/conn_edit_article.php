...
$id_dg_article = $_POST['id_dg_article'];;
...
if ($uploadOk3 == 0) {
  ...
  $name_image3 = mysqli_real_escape_string($db2,$_POST['banner3_lama']);
    ...
    $stmt1 = $db2->prepare("UPDATE `dg_article` set judul_article = ?, banner_utama = ?, isi_article_pembuka = ?, 
    quotes = ?, author_quotes = ?, banner1 = ?, banner2 = ?,
    isi_article = ?, edited_by = ?, edited_at = ? where id_dg_article = ? ");
    $stmt1->bind_param("sssssssssss", $judul_article, $name_image1, $summernote, 
    $quotes, $author_quotes, $name_image2, $name_image3, $summernote2,
    $id_user, $tanggal_now, $id_dg_article);
    ...
    $stmtDC = $db2->prepare("DELETE FROM dg_article_category_m WHERE id_dg_article = ?");
    $stmtDC->bind_param("s", $id_dg_article);

    $stmtDC->execute();
    $stmtDC->close();

    $stmtDT = $db2->prepare("DELETE FROM dg_article_tags_m WHERE id_dg_article = ?");
    $stmtDT->bind_param("s", $id_dg_article);
