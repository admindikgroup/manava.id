...
$query = "SELECT * FROM dg_client_project_type WHERE id_dg_client_project = $id
...
if ($result) {
    ...
    foreach ($data as $row) {
        ...
        echo 'data-c="'.$row['id_dg_client_project_type'].'"';
        echo 'data-d="'.$row['id_dg_client_project'].'"';
        echo 'data-v="'.$row['nama_type'].'"';
...
echo '<input type="hidden" name="id_dg_client_project_to_type" id="id_dg_client_project_to_type" value="'.$id.'">';
